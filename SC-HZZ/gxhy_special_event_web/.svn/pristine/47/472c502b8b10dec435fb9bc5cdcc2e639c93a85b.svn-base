var pollution={
	initPollutionData:function(){
			easyui_datagrid_util.fit=false;
			easyui_datagrid_util.singleSelect=true;
			easyui_datagrid_util.create_datagrid_options("pollution", WEB_ROOT+ "/event/riverCourse/getRiverCourses?type=3",pollution,null);
	},
	formatterPollutionTm:function(value, row, index){
	   return formatter_date_utils.formatterStrTime(value,0,3);
	},
	formatterPollutionDesc:function(value, row, index){
		var content = '';   
		var abValue = value +'';   
		if(value != undefined){      
			if(value.length >= 10) {         
	...
	echartsPiePollution:function(data){
		var myCharts = echarts.init(document.getElementById('pie_pollution'));
		var option = {
				   title : {
				       text: '专题事件分布图',
				       x:'center'
...
$(function(){
	...
	$("#Div0_pollution,#Div1_pollution,#Div2_pollution").height(height - 40);
	$("#pie_pollution").height((height - 40) * 0.5);
	$("#timeAxis").height((height - 40) * 0.5 - 50);
	$("#pollution").height(height - 40);
	pollution.initPollutionData();
	var data=pollution.getPiePollution();
	pollution.echartsPiePollution(data);
});