var mapMroe={
		addRiverOpen:function(data,rvUserData){
			...
			count += "<li class='active'  onclick='mapViewGis.clickWindow(this,0)' ><a href='javascript:void(0)' data-toggle='tab'><i class='fa icon-fire'></i>河流信息</a></li>";
			count += "<li onclick='mapViewGis.clickWindow(this,1)' ><a href='javascript:void(0)' data-toggle='tab'><i class='fa icon-user'></i>河长信息</a></li>";
			count += "<li onclick='mapViewGis.clickWindow(this,4)' ><a href='javascript:void(0)' data-toggle='tab'><i class='fa icon-twitter'></i>公示牌信息</a></li>";
			count += "<li onclick='mapViewGis.clickWindow(this,2)' ><a href='javascript:void(0)' data-toggle='tab'><i class='fa icon-desktop'></i>全景图</a></li>";
			count += "<li onclick='mapViewGis.clickWindow(this,3)' ><a href='javascript:void(0)' data-toggle='tab'><i class='fa icon-picture'></i>图片</a></li>";
			count += "</ul>";
			...
			if(rvUserData != null ){
				...
				for(var i = 0 ;i < rvUserData.length ;i++){
					...
					count += "<p class='hzz_p'><a href='javascript:void(0)'>职务："+rvUserData[i].userJobNm+"</a></p>";
					count += "<p class='hzz_p'>姓名："+rvUserData[i].userNm+"</p>";
					count += "<p class='hzz_p'>所属部门："+rvUserData[i].deptNm+"</p><p class='hzz_p'>联系电话："+rvUserData[i].userPhone+"</p>";
					count += "</div>";
					count += "</div>";
				}
			}
...
//			count += "<div class='demo-image six'  data-image='../img/ny6.jpg' data-title='Chilling sheep' data-caption='To your breed, your fleece, your clan be true!'></div>";
			...
			count += "<p class='gxphzz_p'><a href='javascript:void(0)'>河道名称："+data.rvNm+"</a></p>";
			count += "<p class='gxphzz_p'>河道起点："+data.rvOn+"</p>";
			count += "<p class='gxphzz_p'>河道终点："+data.rvMh+"</p><p class='gxphzz_p'>河道长度："+data.rvlt+" km</p>";
			count += "<p class='gxphzz_p'>县级第一总河长："+"杨兵"+"</p>";
			count += "<p class='gxphzz_p'>县级总河长："+"杨兵"+"</p>";
			count += "<p class='gxphzz_p'>县级副河长："+"杨兵"+"</p>";
			count += "<p class='gxphzz_p'>县级河长："+"杨兵"+"（"+"1371788281"+"）</p>";
			count += "<p class='gxphzz_p'>乡(镇)河长："+"杨兵"+"（"+"1371788281"+"）</p>";
			count += "<p class='gxphzz_p'>村河长："+"杨兵"+"（"+"1371788281"+"）</p>";
			count += "<p class='gxphzz_p'>河长职责：负责流经辖内河道全部清理治理工作</p>";
			count += "</div>";
			count += "</div>";
			count += "</div>";
			count += "</div>";
		...
		appendOpenInfo:function(datas){
			...
			content += "</div><div style='margin: 11px 43px;'><span class='div_span'><i class='fa fa-warning' class='div_span_i'></i>"+(sttp == 'SZ' ? '水质说明':'预警信息')+"<span class='div_span_pd'>|</span ></span ><span class='div_span'  onclick='mapViewGis.localUrl(\""+datas.stcd+"\",\"1\")'><i class='fa fa-history' class='div_span_i'></i>"+(sttp == 'SZ' ? '历史水质':'历史水位')+"<span class='div_span_pd'>|</span ></span><span class='div_span' onclick='mapViewGis.localUrl(\""+datas.stcd+"\",\"0\")'><i class='fa fa-vcard-o' class='div_span_i'></i>测站信息</span></div>";
			content += "</div>";
			return content;
		},
		appendVideoOpenInfo:function(i){
//			var content = "<div style='height:600px;'><iframe style='height:95%;width:100%;' src='http://218.85.131.35:9215/LW_SL2.2/'></iframe>";
			var url  = "";
			if(i == 0){
				url = WEB_ROOT+"/common/video/videoNanYang1.mp4";
			}else if (i == 1){
				url = WEB_ROOT+"/common/video/videoNanYang2.mp4";
			}else{
				url = WEB_ROOT+"/common/video/videoNanYang1.mp4";
			}
	 　		var content =  "<video id='my-video' class='video-js' controls preload='auto' width='700' height='400'";
	 　		content +=  "poster='"+WEB_ROOT+"/common/video/oceans.jpg'>";
