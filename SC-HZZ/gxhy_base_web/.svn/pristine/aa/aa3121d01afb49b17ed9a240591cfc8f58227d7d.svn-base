	...
	detailUser:function(id){
	 ...
	 /*updateUser:function(id){
	  ...
	  //部门
	   ...
	   getCounts:function(){
		   var result= commonUtils.get(WEB_ROOT+"/userManager/work/getCounts",null,null);
		   if(result.flag){
			   $("#xjhz").html(result.data[0]);
			   $("#zjhz").html(result.data[1]);
			   $("#cjhz").html(result.data[2]);
			   $("#xhy").html(result.data[3]);
			   $("#bjy").html(result.data[4]);
			   $("#xhzb").html(result.data[5]);
			   $("#xhz").html(result.data[6]);
			   $("#cydw").html(result.data[7]);
		   }
	   }
   }
...
$(function(){
	...
	$("#patrol_catalogUser").height(p-contextH-userH-60); 
	$("#homeTree").height(p-contextH-userH-50); 
	//初始化加载用户数据
	userManager.getCounts()
